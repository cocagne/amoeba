package org.aspen_ddp.aspen.fs.nfs
...
import org.aspen_ddp.aspen.fs.error.{DirectoryEntryDoesNotExist, DirectoryEntryExists, InvalidInode}
import org.aspen_ddp.aspen.fs.{BaseFile, BlockDevice, CharacterDevice, Directory, DirectoryInode, File, FileHandle, FileMode, FileSystem, FileType, InodePointer, Symlink, Timespec}
...
class AmoebaNFS(val fs: FileSystem,
  ...
  def list(inode: Inode, verifier: Array[Byte], cookie: Long): DirectoryStream = {
    ...
    val entries = blockingCall { dir.getContents() }.zipWithIndex.map { t =>
      val (org.aspen_ddp.aspen.fs.DirectoryEntry(name, iptr), cookie) = t
  ...
  /**
    ...
    */
  @throws[IOException]
  def parentOf(inode: Inode): Inode = getDirectory(inode).inode.asInstanceOf[DirectoryInode].oparent match {
