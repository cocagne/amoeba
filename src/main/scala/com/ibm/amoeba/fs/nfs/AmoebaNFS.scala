package com.ibm.amoeba.fs.nfs

import java.io.IOException
import java.nio.ByteBuffer

import com.github.blemale.scaffeine.{LoadingCache, Scaffeine}
import com.ibm.amoeba.client.{FatalReadError, StopRetrying}
import com.ibm.amoeba.fs.error.{DirectoryEntryDoesNotExist, DirectoryEntryExists, InvalidInode}
...
class AmoebaNFS(val fs: FileSystem,
  ...
  def list(inode: Inode, verifier: Array[Byte], cookie: Long): DirectoryStream = {
    ...
    val entries = blockingCall { dir.getContents() }.zipWithIndex.map { t =>
      val (com.ibm.amoeba.fs.DirectoryEntry(name, iptr), cookie) = t
